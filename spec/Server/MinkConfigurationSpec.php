...
class MinkConfigurationSpec extends ObjectBehavior
{
    function let()
    {
        $this->beConstructedWith(null, null, null, 'http://dev.local:9001/foo/bar', null);
    }
    ...
    {
        $this->beConstructedWith('localhost', null, null, 'http://dev.local:9001/foo/bar', null);
        $this->getHost()->shouldReturn('localhost');
    }

    function it_uses_the_port_from_configuration_when_one_is_provided()
    {
        $this->beConstructedWith(null, 9000, null, 'http://dev.local:9001/foo/bar', null);
    ...
    {
        $this->beConstructedWith(null, 9000, '/var/www', 'http://dev.local:9001/foo/bar', null);
        $this->getDocroot()->shouldReturn('/var/www');
    }

    function it_returns_the_router_it_is_constructed_with()
    {
        $this->beConstructedWith(null, 9000, '/var/www', 'http://dev.local:9001/foo/bar', 'router.php');
